...
> Full suite: `pnpm test` → 155 tests passing (api 54, web 69, ui 32; mobile passWithNoTests).
...
- **Backend API** (`apps/api`) — 13 tRPC routers / 51 procedures (workspace, project, task, reminder, household, person [incl. `delete`], notification, resource, template, search, activity, user, **inbox**). Thin routers + Result-returning services, Zod validation, 3-tier visibility filtering, workspace-membership checks, activity logging.
...
1. **Web screens beyond Dashboard + Inbox + Projects + Household + People** — Calendar, Settings. (Projects ✅, Household ✅, People ✅ done; shared components Input/Modal/Toast/TaskItem/ProjectCard/SegmentedControl ✅ built. Still missing: calendar/settings-specific components. **External calendar/contacts sync (Google/Outlook) to import birthdays is a separate future epic** — see `docs/superpowers/specs/2026-06-13-people-slice-c-design.md` §2.)

...
> Full suite: `pnpm test` → 161 tests passing (api 59, web 70, ui 32; mobile passWithNoTests).
...
- **Backend API** (`apps/api`) — 13 tRPC routers / 52 procedures (workspace, project, task [incl. `reopen`], reminder, household, person [incl. `delete`], notification, resource, template, search, activity, user, **inbox**). Thin routers + Result-returning services, Zod validation, 3-tier visibility filtering, workspace-membership checks, activity logging.
